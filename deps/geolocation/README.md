...
Apple requires a privacy description to be specified in `src-tauri/Info.plist` for location information:
...
<plist version="1.0">
	<dict>
		...
		<string>Describe why your app needs the device's location</string>
	</dict>
</plist>
```

If your app runs in the App Sandbox, you additionally need the location entitlement:
...
The user is prompted the first time your app asks for a position, and can revoke the permission at
any time in `System Settings > Privacy & Security > Location Services`.
...
| `PositionOptions`    | Android                           | iOS     | macOS                      |
| -------------------- | --------------------------------- | ------- | -------------------------- |
| `enableHighAccuracy` | ✓ (needs `coarseLocation` on 12+) | ✓       | ✓                          |
| `timeout`            | ignored for `getCurrentPosition`  | ignored | ✓ for `getCurrentPosition` |
| `maximumAge`         | ✓                                 | ignored | ✓ for `getCurrentPosition` |

On macOS both permission aliases (`location` and `coarseLocation`) always report the same state,
because the system only knows a single location permission.
