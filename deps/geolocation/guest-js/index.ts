...
export type Coordinates = {
   ...
   * Available on macOS, on all iOS versions and on Android 8 and above.
   */
...
export type PermissionStatus = {
   ...
   *
   * On iOS and macOS it requests/checks location permissions.
   */
   ...
   *
   * On iOS and macOS it will have the same value as the `location` alias.
   */
...
export type PositionOptions = {
   ...
   * On macOS this is shared between all active requests, so it is not lowered while a watcher asked for high accuracy.
   */
   ...
   * On macOS the timeout only applies to getCurrentPosition.
   * Ignored on iOS
   */
  timeout: number
  /**
   ...
   * Default: 0
   * On macOS the maximum age only applies to getCurrentPosition.
   * Ignored on iOS
   */
  maximumAge: number
}
